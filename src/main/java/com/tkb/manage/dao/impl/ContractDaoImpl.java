...
//				   + " (SELECT COUNT(*) AS COUNT FROM proposition_manage.lesson_plan LP WHERE C.CONTRACT_ID = LP.CONTRACT_ID AND UPLOAD_STATUS <> 'Y') AS L_COUNT, "
...
//				   + " (SELECT COUNT(*) AS COUNT FROM proposition_manage.proposition P WHERE C.CONTRACT_ID = P.CONTRACT_ID AND QUESTION_TYPE = 1 AND UPLOAD_STATUS <> 'Y') AS P_COUNT_UP, "
...
//				   + " (SELECT COUNT(*) AS COUNT FROM proposition_manage.proposition P WHERE C.CONTRACT_ID = P.CONTRACT_ID AND QUESTION_TYPE = 2 AND UPLOAD_STATUS <> 'Y') AS P_COUNT_UP2 "
...
//				   + " ) A ";
	...
	public Contract data(Contract contract) {
		...
		String sql = " SELECT PMC.ID, PMC.UUID, PMC.TEACHER_ID, PMC.TKB_CONTRACT_NUM, PMC.TKB_CONTRACT_FILE,"
				   ...
				   + " PMC.CSOFE_CONTRACT_NAME, PMC.CSOFE_PARTYA, PMC.CSOFE_PARTYB, PMC.FIELD_ID, PMC.EDUCATION_ID, "
				   ...
				   + " FROM proposition_manage.contract PMC "
				   + " LEFT JOIN proposition_manage.field PMF ON PMF.ID = PMC.FIELD_ID "
	...
	public int add(Contract contract) {
		...
		String sql = " INSERT INTO proposition_manage.contract "
				   ...
				   + " CSOFE_PARTYB, FIELD_ID, EDUCATION_ID, BEGIN_DATE, END_DATE, "
				   + " LESSON_NUM, BASIC_NUM, QUESTIONS_GROUP_NUM, "
				   ...
				   + " :csofe_contract_file, :csofe_contract_name, :csofe_partya, :csofe_partyb, :field_id, "
